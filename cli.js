...
/** Display XRP account balance */
async function printETHBalance({ address, name }) {
  console.log(`${name} XRP balance is`, web3.utils.fromWei(await web3.eth.getBalance(address)))
}
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  if (currency === 'XRP' && refund !== '0') {
    throw new Error('The XRP purchase is supposted to be 0 for XRP withdrawals')
  }
  refund = toWei(refund)
  if (relayerURL) {
    ...
    try {
      ...
      if (netId === 56 || netId === 97) {
        console.log(`Transaction submitted through the relay. View transaction on etherscan https://bscscan.com//tx/${relay.data.txHash}`)
      } else {
    ...
    await smashnado.methods.withdraw(proof, ...args).send({ from: senderAccount, value: refund.toString(), gas: 1e6 })
      .on('transactionHash', function (txHash) {
        if (netId === 56 || netId === 97) {
          console.log(`View transaction on Binance https://bscscan.com/tx/${txHash}`)
        } else {
 ...
 */
async function init({ rpc, noteNetId, currency = 'xrp', amount = '1' }) {
  ...
  if (inBrowser) {
    ...
    try {
      if (web3 == undefined) {
        ...
        contractJson = await (await fetch('build/contracts/XRPSmashnado.json')).json()
    ...
    if (web3 === undefined) {
      ...
      contractJson = require('./build/contracts/XRPSmashnado.json')
      ...
      erc20ContractJson = require('./build/contracts/ERC20Mock.json')
      erc20smashnadoJson = require('./build/contracts/BEP20Smashnado.json')
    }
  }
  ...
  if (!(netId === 56 || netId === 97)) {
    throw new Error('You did not connect to Binance network. Please make sure you connected to BSC Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to BSC Mainnet.')
  erc20 = currency !== 'xrp' ? new web3.eth.Contract(erc20ContractJson.abi, tokenAddress) : {}
}

async function main() {
  if (inBrowser) {
    const instance = { currency: 'xrp', amount: '0.1' }
    //await init(instance)
    ...
    window.deposit = async (_currencyStr, _amountStr) => {
      ...
      try {
        ...
        if (noteString) {
          ...
          resultStr += 'Sender account Matic balance is ' + web3.utils.fromWei(await web3.eth.getBalance(senderAccount)) +'<br/>'
    ...
    program
      .command('syncDB <currency> <amount>')
      .description('Synchronization with DB')
      .action(async (currency, amount) => {
        currency = currency.toLowerCase()
        await syncDB({ rpc: program.rpc, currency, amount })
      })
    try {
